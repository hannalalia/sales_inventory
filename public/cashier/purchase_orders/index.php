...
<?php require('../../../private/shared/staff_navigation.php');?>
    ...
    <table class="mx-auto table table-sm table-hover table-responsive-lg text-center" id="po_table">
    	...
    	<tbody>
    		...
    		<tr>
    			...
    			<td><a class="btn btn-sm bg-transparent text-secondary"  href="<?php echo url_for('cashier/purchase_orders/view.php?id='. $po['purchase_order_id'])?>">
    ...
    <a class="btn btn-info text-light m-2" href="<?php echo url_for('cashier/purchase_orders/new.php')?>" >Create Purchase Order</a>
    <a class="btn btn-info text-light m-2" href="<?php echo url_for('cashier/purchase_orders/po_recon.php')?>" >P.O. Recon</a>
    </div>
<?php require('../../../private/shared/public_footer.php');?>
<script type="text/javascript" src="<?php echo url_for('cashier/resources/js/populate_category.js')?>"></script>
<script >
$(document).ready( function () {
    let row = $('#po_table tr').length;
    if(row>0){
        $('#po_table').DataTable({
            "order": [1,"desc"],
            "columnDefs": [{
...
<script>
    ...
    let date = new Date(due_date_arr[1]+'/'+due_date_arr[2] +'/'+due_date_arr[0]);
        if(today >=date && (status == 'Pending' || status == 'Partially Received' || status == 'Fully Received' )){
